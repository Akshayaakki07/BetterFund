...
export default function CampaignSingle({
  ...
  async function onSubmit(data) {
    ...
    try {
      ...
      const contributionInWei = web3.utils.toWei(data.value, "ether");
      if (parseInt(contributionInWei) <= parseInt(minimumContribution)) {
        setError(`Contribution must be greater than ${web3.utils.fromWei(minimumContribution, 'ether')} ETH`);
        return;
      }
